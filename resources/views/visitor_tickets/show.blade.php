...
@section('title', __('messages.ticket_number') . ' #' . $ticket->id)
...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
    <div class="d-flex align-items-center gap-2">
        <a href="{{ route('visitor-tickets.index') }}" class="btn btn-sm btn-light border rounded-pill px-3 d-flex align-items-center gap-1 text-secondary">
            ...
            <span class="d-none d-sm-inline">{{ __('messages.visitor_ticket_list') }}</span>
        </a>
        <h4 class="mb-0 fw-bold fs-5 fs-md-4">
            {{ __('messages.ticket_number') }} <span class="text-primary">#{{ $ticket->id }}</span>
        </h4>
    </div>
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <a href="{{ route('visitor-tickets.edit', $ticket->id) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3 py-1 d-inline-flex align-items-center gap-1 shadow-sm">
        ...
        <form action="{{ route('visitor-tickets.destroy', $ticket->id) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('messages.delete_ticket_confirm') }}');">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-sm btn-outline-danger rounded-pill px-3 py-1 d-inline-flex align-items-center gap-1 shadow-sm">
        ...
        <div class="card border-0 shadow-sm rounded-4">
            ...
            <div class="card-body p-3 p-md-4">
                <div class="row g-2 g-md-3">
                    ...
                    <div class="col-12">
                        <div class="text-secondary small fw-semibold">{{ __('messages.purpose') }}</div>
                        <div class="fw-medium text-dark">{{ $ticket->purpose }}</div>
                    </div>
                    @if($ticket->isClosed())
                    <div class="col-6 col-md-6 border-top pt-2 mt-2">
                        <div class="text-secondary small fw-semibold">{{ __('messages.closed_at') }}</div>
                        <div class="small fw-semibold text-muted">{{ $ticket->closed_at ? $ticket->closed_at->format('d/m/Y H:i') : '—' }}</div>
                    </div>
                    <div class="col-6 col-md-6 border-top pt-2 mt-2">
                        <div class="text-secondary small fw-semibold">{{ __('messages.closed_by') }}</div>
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-transparent border-bottom px-4 pt-3 pt-md-4 pb-3 d-flex justify-content-between align-items-center">
                <h6 class="fw-bold mb-0 d-flex align-items-center gap-2">
                    ...
                    <span class="badge bg-primary bg-opacity-10 text-primary ms-1 fw-semibold">{{ __('messages.guest_count_people', ['count' => $ticket->guests->count()]) }}</span>
                </h6>
            </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light border-bottom">
                            <tr class="small fw-semibold text-secondary text-uppercase">
                                <th class="px-4 py-3" style="width:50px">{{ __('messages.stt') }}</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-bottom">
                                ...
                                {{-- Kiểm tra hành lý (Người tạo phiếu đã chọn) --}}
                                <td class="text-center">
                                    @if(is_null($guest->baggage_checked))
                                        <span class="text-muted small">—</span>
                                    @elseif($guest->baggage_checked)
                                        <span class="badge rounded-pill" style="background:#dcfce7;color:#15803d;font-size:0.8rem;">✓ {{ __('messages.yes') }}</span>
                                    @else
                                        <span class="badge rounded-pill" style="background:#fef2f2;color:#dc2626;font-size:0.8rem;">✗ {{ __('messages.no') }}</span>
                                    @endif
                                </td>

                                {{-- Số thẻ khách (Bảo vệ cấp phát) --}}
                                <td>
                                    @if($canSecurity && $ticket->isOpen())
                                ...
                                <td>
                                    @if($canSecurity && $ticket->isOpen())
                                        <div class="d-flex gap-1 align-items-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline-secondary btn-now rounded-2" onclick="setNow('desk_in_{{ $i }}', 'mob_in_{{ $i }}')" title="{{ __('messages.get_current_time') }}">⏱️</button>
                                        </div>
                                    @else
                                ...
                                <td>
                                    @if($canSecurity && $ticket->isOpen())
                                        <div class="d-flex gap-1 align-items-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline-secondary btn-now rounded-2" onclick="setNow('desk_out_{{ $i }}', 'mob_out_{{ $i }}')" title="{{ __('messages.get_current_time') }}">⏱️</button>
                                        </div>
                                    @else
                            ...
                            @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4 small">{{ __('messages.no_guests_in_ticket') }}</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        <div class="guest-mobile-card p-3 border">
                            ...
                            <div class="d-flex justify-content-between align-items-start border-bottom pb-2 mb-3">
                                <div>
                                    ...
                                    <div class="text-muted small mt-1">
                                        {{ __('messages.id_number') }}: <span class="fw-semibold text-dark">{{ $guest->id_number ?: '—' }}</span>
                                    </div>
                                </div>
                                <div class="text-end">
                                    <span class="text-secondary small d-block mb-1">{{ __('messages.baggage_checked') }}:</span>
                                    @if(is_null($guest->baggage_checked))
                                        <span class="text-muted small">—</span>
                                    @elseif($guest->baggage_checked)
                                        <span class="badge rounded-pill" style="background:#dcfce7;color:#15803d;">✓ {{ __('messages.yes') }}</span>
                                    @else
                                        <span class="badge rounded-pill" style="background:#fef2f2;color:#dc2626;">✗ {{ __('messages.no') }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-12 col-sm-6 mb-2">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <label class="form-label small fw-bold text-success mb-0">{{ __('messages.checked_in_at') }}:</label>
                                ...
                                <div class="col-12 col-sm-6 mb-2">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <label class="form-label small fw-bold text-danger mb-0">{{ __('messages.checked_out_at') }}:</label>
                                ...
                                {{-- Ghi chú --}}
                                <div class="col-12">
                                    <label class="form-label small fw-bold text-secondary mb-1">{{ __('messages.note') }}:</label>
                                    <input type="text" class="form-control form-control-sm"
                                           name="guests[{{ $i }}][note]"
                                           value="{{ $guest->note }}"
                            ...
                            <div class="bg-light p-2 rounded-3 border small">
                                <div class="row g-2">
                                    <div class="col-6">
                                        <span class="text-secondary">{{ __('messages.guest_card_number') }}:</span>
                                        <span class="fw-bold text-dark">{{ $guest->guest_card_number ?: '—' }}</span>
                                    </div>
                                    <div class="col-6">
                                        <span class="text-secondary">{{ __('messages.baggage_checked') }}:</span>
                                        @if(is_null($guest->baggage_checked))
                                            <span class="text-muted">—</span>
                                        @elseif($guest->baggage_checked)
                                            <span class="text-success fw-bold">✓ {{ __('messages.yes') }}</span>
                                        @else
                                            <span class="text-danger fw-bold">✗ {{ __('messages.no') }}</span>
                                        @endif
                                    </div>
                                    <div class="col-6">
                                        <span class="text-secondary">{{ __('messages.checked_in_at') }}:</span>
                                        <span class="{{ $guest->checked_in_at ? 'fw-bold text-success' : 'text-muted' }}">
                                            {{ $guest->checked_in_at ? $guest->checked_in_at->format('H:i d/m') : '—' }}
                                        </span>
                                    </div>
                                    <div class="col-6">
                                        <span class="text-secondary">{{ __('messages.checked_out_at') }}:</span>
                                        <span class="{{ $guest->checked_out_at ? 'fw-bold text-danger' : 'text-muted' }}">
                                            {{ $guest->checked_out_at ? $guest->checked_out_at->format('H:i d/m') : '—' }}
                                        </span>
                                    </div>
                                    <div class="col-12">
                                        <span class="text-secondary">{{ __('messages.note') }}:</span>
                        ...
                        @empty
                        <div class="text-center py-4 text-muted small">{{ __('messages.no_guests_in_ticket') }}</div>
                        @endforelse
                    </div>
                </div>
                ...
                @if($canSecurity && $ticket->isOpen())
                    ...
                    <div class="px-3 px-md-4 py-3 border-top bg-light d-flex gap-2 flex-column flex-md-row align-items-stretch align-items-md-center justify-content-between">
                        <div class="text-muted small d-none d-md-block">
                            ...
                            {{ __('messages.security_update_hint') }}
                        </div>
                        <button type="submit" class="btn btn-primary btn-lg py-3 py-md-2 rounded-3 px-4 fw-bold shadow-sm d-flex align-items-center justify-content-center gap-2 tap">
                            ...
                            {{ __('messages.save_security_info') }}
                        </button>
                    </div>
                </form>
                @endif
    ...
    <div class="col-12">
        <div class="card border-0 shadow-sm rounded-4" style="background:#fff5f5; border: 1px solid #fecaca !important;">
            <div class="card-body p-3 p-md-4 d-flex flex-column flex-md-row align-items-stretch align-items-md-center justify-content-between gap-3">
                <div>
                    <div class="fw-bold text-danger fs-6 mb-1">{{ __('messages.close_ticket_title') }}</div>
                    <div class="text-muted small">{{ __('messages.close_ticket_desc') }}</div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 border-0 shadow-lg">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-bold text-danger">{{ __('messages.confirm_close_ticket') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body pt-3">
                    <p class="text-muted">{{ __('messages.confirm_close_ticket_desc', ['id' => $ticket->id, 'unit' => $ticket->guest_unit]) }}</p>
                    <div class="alert alert-warning border-0 rounded-3 small py-2">
                        ...
                        {{ __('messages.confirm_close_ticket_warning') }}
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-light rounded-3 px-3" data-bs-dismiss="modal">{{ __('messages.cancel') }}</button>
                    <form method="POST" action="{{ route('visitor-tickets.close', $ticket->id) }}" class="d-inline">
                        ...
                        <button type="submit" class="btn btn-danger rounded-3 fw-bold px-4 py-2">
                            {{ __('messages.confirm_close_ticket') }}
                        </button>
                    </form>
                </div>
            </div>
