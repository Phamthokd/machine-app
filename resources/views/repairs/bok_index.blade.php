...
@section('title', __('messages.bok_history'))
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="mb-0 fw-bold">{{ __('messages.bok_history_list') }}</h4>
            <div class="text-secondary small">{{ __('messages.bok_history_subtitle') }}</div>
        </div>
    </div>
    
    <div class="d-flex gap-2">
        <a href="/repairs/bok/export?{{ http_build_query(request()->query()) }}" class="btn btn-success text-white shadow-sm fw-bold d-flex align-items-center gap-2">
...
<div class="card-modern p-4 mb-4">
    <form method="GET" action="/repairs/bok" class="row g-3 align-items-end">
        <div class="col-md-3">
        ...
        <div class="col-md-3 d-flex gap-2">
            ...
            <a href="/repairs/bok" class="btn btn-outline-secondary d-flex align-items-center justify-content-center" style="width: 42px; border-radius: 8px;" title="{{ __('messages.clear_filter') }}">
...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0" style="min-width: 900px; font-size: 0.9rem;">
            <thead class="bg-light text-secondary">
                <tr class="text-uppercase text-xs fw-bold">
                    <th class="py-3 px-3" style="width: 60px;">STT</th>
                    <th class="py-3 px-3" style="width: 180px;">{{ __('messages.machine_label') }}</th>
                    <th class="py-3 px-3">Mô tả lỗi</th>
                    <th class="py-3 px-3">Nguyên nhân hư hỏng</th>
                    <th class="py-3 px-3">Biện pháp khắc phục</th>
                    <th class="py-3 px-3" style="width: 180px;">{{ __('messages.time') }}</th>
                    <th class="py-3 px-3" style="width: 150px;">{{ __('messages.repairer') }}</th>
                    ...
                    <td class="px-3">
                        <div class="d-flex flex-column">
                            <span class="fw-bold text-primary">
                                <a href="/repairs/{{ $r->id }}" class="text-decoration-none">{{ $r->machine->ma_thiet_bi ?? '—' }}</a>
                            </span>
                ...
                <tr>
                    <td colspan="7" class="text-center py-5 text-secondary">
                        <div class="mb-3 opacity-50">
                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
                        </div>
